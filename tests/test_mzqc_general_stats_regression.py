...
def test_general_stats_removes_run_level_tolerance_columns() -> None:
    data = {
        "run-a": {
            "raw_precursor": 16.42,
            "raw_fragment": 22.58,
    ...
    assert set(filtered_headers) == {"ms1"}
...
def test_group_general_stats_payload_is_complete_and_numeric() -> None:
    runs = [
    ...
    assert data["run-b"] == data["run-a"]
    assert data["run-c"] == {
        "prideqc_group_precursor_tolerance_ppm": 14,
        "prideqc_group_fragment_tolerance_ppm": 7,
        "prideqc_experiment_group": 2,
    }
    assert headers["prideqc_group_precursor_tolerance_ppm"]["title"] == (
        "Precursor median tol."
    )
    assert headers["prideqc_group_fragment_tolerance_ppm"]["title"] == (
        "Fragment median tol."
    )
    assert headers["prideqc_experiment_group"]["format"] == "{:,.0f}"
    assert headers["prideqc_experiment_group"]["scale"] is False


def test_group_general_stats_keeps_group_id_when_tolerance_is_unavailable() -> None:
    ...
    assert data["run-a"] == {"prideqc_experiment_group": 2}
    assert "prideqc_group_precursor_tolerance_ppm" not in headers
    assert "prideqc_group_fragment_tolerance_ppm" not in headers
    assert "prideqc_experiment_group" in headers
