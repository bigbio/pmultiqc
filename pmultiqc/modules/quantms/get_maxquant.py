...
# 解析原始数据, 预处理
def read(file_path, file_type=None, filter=None):
    MQ_data = pd.read_csv(file_path, sep='\t', low_memory=False)

    # print('maxquant read():', MQ_data.shape)
    ...
    if file_type == 'protgroup':
        ...
        # TODO 数据类型检查, Intensity 相关列是否为数值型
        
        # TODO PTXQC 中增加了一列, 值为 Intensity / Mol. weight [kDa]
        # 命名为 "intensity" + "AbInd" + 列名, 后续查看该参数是哪里用到的
        if 'Mol. weight [kDa]' in MQ_data.columns:
            # 取出所有涉及 Intensity 的列名
            intensity_cols = [col for col in MQ_data.columns if re.search(r'intensity', col, re.IGNORECASE)]
            # print('length of intensity_cols:', len(intensity_cols))

            # PTXQC 中是把 intensity 替换为了 AbInd
            new_intensity_cols = [re.sub(r"intensity", 'AbInd', col, flags=re.IGNORECASE) for col in intensity_cols]
            # print('length of new_intensity_cols:', len(new_intensity_cols))
    ...
    if file_type == 'summary':
        ...
        # 如果有非空元素的个数在2个及以下的行, 则需要排除
        row_index = MQ_data.notna().sum(axis=1) > 2
        # 最后一行为 Total 去掉, 设置为 False
    ...
    # if file_type == 'evidence':
        ...
        # PTXQC 中有三个检查条件:
        #   1. Type 和 Modified sequence 是否存在于 evidence.txt 的列中
        #   2. Type 列中是否包含 'MULTI-MATCH'
        #   3. 'MULTI-MATCH' 对应行中的 Modified sequence 是否为 空
        # 目的是为了检查 'MULTI-MATCH' 所在行对应的 'Modified sequence' # 不能为空
...
def get_protegroups(file_path):
    ...
    # 提取
    MQ_data = read(file_path, 'protgroup', filter='Reverse')

    if not all(column in MQ_data.columns for column in ['Potential contaminant']):
        raise ValueError('Missing required columns in "proteinGroups.txt"!')
    
    # PTXQC 中使用一个 MAP_pg_groups 用于组别缩写名称的对应
    # TODO proteinGroup 中每个 实验组 名称的缩写
    ...
    ## 'LFQ intensity'
    # TODO 在 'LFQ intensity' 不存在的情况下, 需要展示 'Reporter intensity' 列的数据
...
# 1-1. PG:~Contaminants
# TODO 需要考虑的是: 如何在比值中体现总的蛋白丰度, 即不同组之间的蛋白总丰度的差异
...
def get_evidence(file_path):
    ...
    MQ_data = read(file_path, filter='Reverse')

    if not all(column in MQ_data.columns for column in ['Intensity', 'Potential contaminant', 'Raw file', 'Proteins', 'Charge', 
                                                        'Modifications', 'Retention time', 'Retention length', 'MS/MS Count', 
                                                        'Modified sequence', 'Type', 'Calibrated retention time', 
                                                        'Uncalibrated Mass Error [ppm]', 'Mass Error [ppm]', 'Protein group IDs']):
        raise ValueError('Missing required columns in "evidence.txt"!')
...
def evidence_top_contaminants(evidence_df, Top_N):
    ...
    # 取出 contaminant 涉及的所有行
    contaminant_df = evidence_data[evidence_data['Potential contaminant'] == '+']

    # 按 protein_name 分组，计算每组的 intensity 之和，并除以总的Intensity sum_intensity 后乘以 100
    ...
    # 计算每个 Raw File 对应 contaminant Intensity 的总和
    ...
    # 把每个 Raw file 对应总的 Intensity 合并
    ...
    # 这里直接缩写名称的话, 会引起重复的情况, 在字典中不能有重复的 keys
    # 蛋白名称缩写, 超过 15 个字符的只取前 13 个字符
...
def evidence_uncalibrated_mass_error(evidence_data):
    ...
    # summary_slice = summary_df[['Raw file', 'MS/MS Identified [%]']].copy()
    # # display(summary_slice)
    ...
    # evidence_slice = evidence_data[evidence_data['Potential contaminant'] != '+'][['Raw file', 'Uncalibrated Mass Error [ppm]', 'Mass Error [ppm]']].copy()
    # # display(evidence_slice)

    # MS1_decal_smr = pd.DataFrame()
    # for raw_file, group in evidence_slice.groupby('Raw file'):
    #     group_df = pd.DataFrame([[raw_file, 
    #                             len(group), 
    #                             round(group['Mass Error [ppm]'].std(), 1), 
    #                             round(group['Uncalibrated Mass Error [ppm]'].std(), 1), 
    #                             group['Mass Error [ppm]'].quantile([0.01, 0.99]).diff().iloc[-1], 
    #                             group['Uncalibrated Mass Error [ppm]'].apply(abs).median() > 1000]], 
    #                             columns=['Raw file', 
    #                                     'n', 
    #                                     'sd_mass', 
    #                                     'sd_uncalibrated_mass', 
    #                                     'range', 
    #                                     'decal'
    #                                     ])
    #     MS1_decal_smr = pd.concat([MS1_decal_smr, group_df], ignore_index=True)

    # # display(MS1_decal_smr)

    # pd.merge(MS1_decal_smr, summary_slice, on='Raw file')
...
def get_msms(file_path, evidence_df=None):
    ...
    if not all(column in MQ_data.columns for column in ['Evidence ID', 'Raw file', 'Missed cleavages']):
        raise ValueError('Missing required columns in "msms.txt"!')

    if evidence_df is None:
        raise ValueError('Missing evidence dataframe!')
...
def msms_missed_cleavages(msms_df, evidence_df=None):
...
def msmsScans_ion_injec_time_RT(msmsScans_df):
    ...
    mean_ion_injec_time_df['int_mean_ion_injection_time'] = mean_ion_injec_time_df['mean_ion_injection_time'].apply(lambda x: int(x))
