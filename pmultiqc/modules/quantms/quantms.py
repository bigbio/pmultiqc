...
class QuantMSModule(BaseMultiqcModule):
    ...
    def __init__(self):
        ...
        if config.kwargs.get('parse_maxquant', False):
            ...
            # msmsScans.txt
            log.info("{}: Parsing msmsScans file {}...".format(datetime.now().strftime("%H:%M:%S"), self.maxquant_paths['msmsScans']))
            get_msmsScans_dicts = get_maxquant.get_msmsScans(file_path=self.maxquant_paths['msmsScans'])
            log.info("{}: Completed the processing of the msmsScans file {}...".format(datetime.now().strftime("%H:%M:%S"), self.maxquant_paths['msmsScans']))
