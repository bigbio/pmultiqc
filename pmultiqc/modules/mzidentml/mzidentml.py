...
class MzIdentMLModule(BasePMultiqcModule):
    ...
    def get_data(self) -> bool | None:
        ...
        try:
            self.log.info("[TEST-mzQC] Initializing test export for mzIdentML...")
            
            # 1. Package the relevant raw parameters from this module run
            raw_payload = {
                "protein_group_count": self.total_protein_identified,
                "total_peptide_ids": self.total_peptide_count,
                "total_psm_count": self.total_ms2_spectra_identified,
                "missed_cleavages": self.quantms_missed_cleavages,
                "charge_state": self.heatmap_charge_score,
                "identified_peptides_over_rt": self.id_rt_score,
                "peptide_modification_frequency": self.quantms_modified,
                "fdr_threshold_applied": 0.01,  # Test value
                "target_decoy_ratio": 1.0       # Test value
            }


            from pmultiqc.export.mzqc_exporter import MzQcExporter  
            exporter = MzQcExporter(
                pipeline_name="mzidentml", 
                raw_data=raw_payload, 
                output_dir="./test_output" 
            )

            # 3. Process registry dictionary to QualityMetric items
            metrics_list = exporter._parse_mzidentml(raw_payload)

            # 4. Generate and save the file
            test_file_path = exporter.export_to_file(metrics_list, filename="test_mzidentml.mzQC")
            self.log.info(f"[TEST-mzQC] Export successful! File saved to: {test_file_path}")

        except Exception as test_error:
            self.log.error(f"[TEST-mzQC] Verification failed with error: {test_error}", exc_info=True)
